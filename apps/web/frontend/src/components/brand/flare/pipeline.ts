...
// biome-ignore-all lint/style/noMagicNumbers: Flare shader coefficients and GPU texture layout.
// biome-ignore-all lint/complexity/useMaxParams: Retain the upstream GPU pipeline signatures.
...
const FLARE_COLOR = [1, 0.87, 0.7] as const;
...
export class FlarePipeline {
  ...
  constructor(
    ...
    try {
      this.effects = {
        logo: effect(gpu, fullscreen(logoWgsl), { label: 'open-sync-flare-logo' }),
        rim: effect(gpu, fullscreen(rimWgsl), { label: 'open-sync-flare-rim' }),
        rimBlurH: effect(gpu, fullscreen(blurWgsl), {
          label: 'open-sync-flare-rim-horizontal',
        }),
        rimBlurV: effect(gpu, fullscreen(blurWgsl), {
          label: 'open-sync-flare-rim-vertical',
        }),
        composite: effect(gpu, fullscreen(compositeWgsl), {
          label: 'open-sync-flare-composite',
        }),
      };
    } catch (error) {
  ...
  async replace(
    ...
    try {
      ...
      const nextTexture = this.gpu.gpu.createTexture({
        label: 'open-sync-flare-logo-raster',
      ...
      try {
        ...
        this.restore({ size: previousOutputSize, targets: previousTargets, logo: previousLogo });
        throw error;
      }
  ...
  private restore({
    ...
    size: Point;
    targets: FlareTargets | undefined;
    logo: BoundLogo | undefined;
  }) {
    bestEffort(() => this.output.resize(size));
    if (targets) {
      bestEffort(() => this.bindTargets(targets));
    }
    if (logo) {
      bestEffort(() => this.bindLogo(logo));
    }
  }
...
export function backingDimensions(width: number, height: number, dpr: number): Point {
  const pixelRatio = Math.min(
    Math.max(dpr, 1),
    1.75,
    MAX_RENDER_DIMENSION / Math.max(width, height, 1),
  );
  return [
...
function createBlueNoiseTexture(gpu: Gpu): GPUTexture {
  const texture = gpu.gpu.createTexture({
    label: 'open-sync-flare-blue-noise-128',
