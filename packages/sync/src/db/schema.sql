...
CREATE TABLE IF NOT EXISTS sync_runs (
  ...
  records_processed INTEGER NOT NULL DEFAULT 0, records_queued INTEGER NOT NULL DEFAULT 0,
  failure_count INTEGER NOT NULL DEFAULT 0, error_code TEXT,
  PRIMARY KEY(owner_id, id),
  FOREIGN KEY(owner_id, sync_id) REFERENCES syncs(owner_id, id)
);
CREATE UNIQUE INDEX IF NOT EXISTS one_open_run ON sync_runs(owner_id, sync_id) WHERE completed_at IS NULL;
